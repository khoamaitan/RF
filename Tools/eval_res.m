function [ conv_res,mat_stable ] = eval_res( ures_old,vres_old,ures,vres,abs_ures,abs_vres )
...
conf_abs = conf_abs_ures.*conf_abs_vres;
conf_diff = max(0,(abs(ures_old)-abs(ures))./(abs(ures_old)+eps)).*...
    max(0,(abs(vres_old)-abs(vres))./(abs(vres_old)+eps));

conv_res=(conf_abs+conf_diff);
max_val = max(conv_res(:));
conv_res=conv_res./max_val;
end
