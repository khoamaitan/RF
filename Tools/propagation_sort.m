function [ uvklt,w ] = propagation_sort( uvklt,w,Patches,check_mat,img1c,index )
%Propagate all points based on the patches
%Propagate from higest to lowest
H=size(uvklt,1);
W=size(uvklt,2);
h_wsize= 0;
n_point = sum(Patches(:));
n_all_point=length(index);
cas=1;
switch cas
    case 1
        %Case 1 compute all points
        %fprintf('Propagation \n');
        for k =2:floor(n_all_point*1)
            inum=index(k);
            j = floor(((inum-1)/H))+1;
            i = inum-(j-1)*H;
