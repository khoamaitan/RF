function [ Patches,check_mat,ind_p ] = create_patch_nol( w,w_size )
%Localize the NON-OVERLAPPING confidence patch
...
for i=1:np
    ...
    if ((row-2*h_wsize) > 0) &&  ((row+2*h_wsize) < (H+1)) &&...
        ...
        s_p = check_mat(row-2*h_wsize:row+2*h_wsize,col-2*h_wsize:col+2*h_wsize);
        
        
        if (sum(s_p(:)) == 0 ) %If available to create patch:
