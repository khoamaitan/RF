...
%% Load image and GT
clear all;
add2='C:\Users\MAI\Dropbox\Database\optical flow\';
...
%     imshow(tup);
    ...
    for k=1:nb_wrap(lvl)
        %% Warping Image
        [Ix,Iy,It,warpImc]=partial_derivation(pyr_image1,pyr_image2,uvklt,1);
        [x,y]   = meshgrid(1:W,1:H);
        x2      = x + uvklt(:,:,1);
        y2      = y + uvklt(:,:,2);
        idxx = max(1,min(W,x2));
        ...
        taux= makemtrx_prop(H,W,warpImc);
        
%% KLT Flow
        %fprintf('KLT ... ')
        [ures,vres,minEig,iD]=LKT_res(Ix,Iy,It,maxh,maxw);
%        if lvl ==1 
